...
export function Button({ icon: Icon, name, url, type }: ButtonProps) {
  const onClick = () => {
    window.open(url, '_blank');
  };

  const baseClasses =
    'font-manrope group relative flex items-center justify-center rounded-full border border-[var(--soft-light-gray)] bg-white p-2 text-[var(--black)] transition-all duration-500 ease-in-out';
  ...
  const renderText = () => {
    if (type === 'text-only' || type === 'full-static') {
      return <span className="ml-2">{name}</span>;
    }
